...
请根据这些原则，创作一个跑团剧本。避免直接使用真实事件，除非用户要求使用真实故事背景,但可以借鉴现实因素。重点是让玩家在游戏过程中能反思现实处境，获得启发性体验。
每次生成都需要输出一份完整的跑团剧本
"""
...
需要生成不少于3个第一幕文本，每个第一幕文本,需要交代完整的故事背景，需要有不同的情节发展，生成的文本要具有沉浸感，且文本的结束是轮到主角做出选择。
